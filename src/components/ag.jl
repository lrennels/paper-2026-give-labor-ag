...
@defcomp Agriculture begin
    ...
    # Parameters
    gdp90 = Parameter(index=[time], unit="billion US\$2005/yr")
    gdp = Parameter(index=[time, country], unit="billion US\$2005/yr")
    population90 = Parameter(index=[time], unit = "million")
    ...
    function run_timestep(p,v,d,t)
        for c in d.countries
            ...
            ypc90 = (1_000 * p.gdp90[c]) / p.pop90[c] # USD per person
            ...
            impact = linear_interpolate([0, p.gtap_df[c, :]...], collect(0:0.5:7), p.temp[t])
            v.agloss_gtap[t, c] = -(impact / 100) # take the negative to go from impact to loss
